...
public class Extras {
    ...
    public static String guideText() {
        return "【基础】点=戳（嗯？）；连点三下=拍扁（2 秒复活）；按住 0.65s=摸头 +好感；长按 1.5s=打开设置；拖动=搬家；甩出=坠落+眩晕；双指捏合=缩放。\n"
                ...
                + "【饲养】饱食度按真实时间流逝下降（白天约 12.6/小时，夜里 22-7 点掉 1.6 倍；关掉 App 的时间也照算）；血池随时间回复（白天约 7.9/小时）。点「献血」立刻吸一口，无冷却、不挑食，随你喂几次；每口吃掉当前血池的 40%~100%，吸多少血就涨多少饱食度，18% 暴击双倍，顶到 100 会满足地打个嗝。\n"
