...
public class Extras {
    ...
    public static String guideText() {
        return "【基础】点=戳（嗯？）；连点三下=拍扁（2 秒复活）；按住 0.65s=摸头 +好感；长按 1.5s=打开设置；拖动=搬家；甩出=抛物线扔出去（甩得越快飞得越远，左右墙会弹，落地晕一会）；双指捏合=缩放。\n"
                + "【状态】cruise 巡航/dash 冲刺/zigzag 之字/沿边巡逻/角落休息/探头/绕圈/翻筋斗/俯冲/8字/蝴蝶飘/打盹(22-7点)/眩晕等 16+ 种。靠近站立线（离底 2% 内）会慢慢靠过去站住、再连跳 1~10 下（每下 0.5~2 秒随机）、然后飞走；勿扰模式则是慢慢沉到线上静静站着。\n"
                + "【高度】设置页有两根滑块：①「跳跃高度」＝一下跳多高（屏高 4%~40%，默认 14%）；②「站立高度」＝它站在哪一层，即离屏幕底部多高（0%~50%，默认 0% 就是贴底）。站立线只决定它在哪站：jump 模式的站位、底部连跳靠过去的那条线、勿扰站桩都按它来；拖动搬家、平时飞行、被甩出去的落点都不受影响。\n"
                + "【模式】屏幕右缘拉手点开即可一键切换「专注 / 勿扰 / 工作 / jump」（开着的会标 ●）。jump 模式下它会站在站立线上不动，拖它可以横向挪位置，点一下跳两下。\n"
