...
public class Extras {
    ...
    public static String guideText() {
        return "【基础】点=戳（嗯？）；连点三下=拍扁（2 秒复活）；按住 0.65s=摸头 +好感；长按 1.5s=打开设置；拖动=搬家；甩出=坠落+眩晕；双指捏合=缩放。\n"
                ...
                + "【饲养】饱食度按真实时间流逝下降（白天约 12.6/小时，夜里 22-7 点掉 1.6 倍；关掉 App 的时间也照算），设置页有进度条可视化。点「献血」可以无限点、没有冷却，血池空了也照样能喂；每口随机吸 15~40 血，吸多少血涨多少饱食度，18% 暴击双倍。但蚊子有约 20% 概率耍脾气拒绝（挑食/不饿），拒绝就不消耗、不涨。顶到 100 会满足地打个嗝。\n"
